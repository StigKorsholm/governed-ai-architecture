...
* **Decision:** All document extractors — regardless of tool or approach — output a common canonical schema (Legacy System Template).
...
But ensemble extraction requires a common language. If each approach outputs a different format, comparison becomes an N×M integration problem.

**Tool choice is implementation.** Whether you use Docling, Azure Document Intelligence, AWS Textract, LlamaParse, marker-pdf, or custom scripts — the canonical schema is the same. What matters is that you vary your *approach*, not your *vendor*.

---

## Decision Drivers

1. **Approach-agnostic validation** — Validators should not know or care which tool or approach produced the output
2. **Cross-approach comparison** — Must detect when approaches agree or disagree on the same entity
3. **Completeness measurement** — Must calculate coverage against declared inventory (TOC, screen indexes)
4. **Evidence provenance** — Must trace every extracted entity to source page and text
5. **Graph compatibility** — Output must be transformable to graph nodes/edges without loss
6. **Extensibility** — New entity types (e.g., BatchJob) should not break existing extractors

---

## Considered Options

### Option 1 — Tool-specific schemas
...
### Option 3 — Canonical extraction schema ✅ **Chosen**
...
```
[Approach A] ──┐
[Approach B] ──┼──► [Legacy System Template] ──► [Validators] ──► [Graph]
[Approach C] ──┘
```
...
screens:
  - id: "1.1"
    name: "Forespørgsel korrespondent"
    evidence:
      page: 27
      confidence: 0.95
      source_text: "1.1 Forespørgsel korrespondent"
```
...
Note: Declared inventory represents the system's best extracted claim of completeness, not ground truth. Coverage metrics are therefore relative to what the document claims, not absolute.
...
The canonical extraction schema is authoritative only for staging and validation; semantic authority remains defined by graph invariants and domain rules (ADR-007).
...
Running the same tool with the same settings twice adds no information. Running different approaches — even with the same underlying tool — creates the variation needed for meaningful comparison.
...
| Graph ingestion transforms template → nodes/edges | Covered by existing graph pipeline |
| Tool selection is decoupled from architecture | Teams can swap tools without ADR changes |

### Positive Consequences

- Approach-agnostic validation — add new tools without validator changes
- Measurable completeness — coverage ratio is a number, not a feeling
- Cross-approach consensus — ensemble extraction becomes possible
- Clear staging area — human review before graph commit
- Audit trail — template instances are artifacts (ADR-003)
- Tool flexibility — swap Docling for Azure DI tomorrow, schema unchanged
...
- Partial extraction handling (accept / WARN / STOP) is governed by validator thresholds and human checkpoints (ADR-009, ADR-008)

### What Breaks Without This Decision

| Failure Mode | Consequence |
|--------------|-------------|
| Validators become tool-specific | N validators × M tools = combinatorial complexity |
...
- **Schema definition** (YAML/JSON Schema) is implementation — will live in MIP repo under `/schemas/`
...
- **Schema versions** are immutable per run; upgrades require explicit version migration (aligned with ADR-003)
...
- **ADR-009:** Migration STOP Conditions
